module cnn_mmio_controller (
    ...
    localparam [31:0] CNN_BASE       = 32'h0300_0000;
    ...
    wire selected = iomem_valid && (iomem_addr[31:8] == CNN_BASE[31:8]);
    wire bus_write = selected && (iomem_wstrb != 4'b0000);

    assign iomem_ready = selected;
    ...
    always @* begin
        case (iomem_wstrb)
            4'b0001: selected_input_byte = iomem_wdata[7:0];
            4'b0010: selected_input_byte = iomem_wdata[15:8];
            4'b0100: selected_input_byte = iomem_wdata[23:16];
            4'b1000: selected_input_byte = iomem_wdata[31:24];
            default: selected_input_byte = 8'h00;
        endcase
    end

    wire input_write_accept =
        bus_write && (iomem_addr[7:0] == 8'h18) &&
        input_strobe_onehot && (state == ST_IDLE) && !busy && !input_full;
    ...
    always @* begin
        case (iomem_addr[7:0])
            8'h00: iomem_rdata = CNN_ID_VERSION;
            8'h04: iomem_rdata = {23'd0, irq_enable, 8'd0};
            8'h08: iomem_rdata = {
                27'd0,
                input_full,
                error,
            ...
            8'h0C: iomem_rdata = CNN_CONFIG;
            8'h10: iomem_rdata = 32'd784;
            8'h14: iomem_rdata = {22'd0, input_count};
            8'h18: iomem_rdata = 32'd0;
            8'h1C: iomem_rdata = {28'd0, result_class};
            8'h20: iomem_rdata = cycle_count;
            8'h24: iomem_rdata = error_code;
            8'h28: iomem_rdata = frame_id;
    ...
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            // Normally released. CONTROL.CLEAR or timeout asserts the CNN
            // reset for exactly one clock.
            core_resetn <= 1'b1;
            ...
            case (state)
                ...
                ST_COOLDOWN: begin
                    // The sample core clears at sequencer count 1280 and
                    // raises tready again after the pipeline flush.
                    if (core_s_ready)
                        state <= ST_IDLE;
                end
            ...
            if (bus_write && (iomem_addr[7:0] == 8'h18)) begin
            ...
            if (bus_write && (iomem_addr[7:0] == 8'h04)) begin
                irq_enable <= iomem_wdata[8];

                // bit 1: clear/abort everything and reset the CNN pipeline.
                if (iomem_wdata[1]) begin
                    state        <= ST_IDLE;
                    input_count  <= 10'd0;
                    feed_index   <= 10'd0;
                    stream_valid <= 1'b0;
